...
shinyServer(function(input, output, session) {
  ...
  output$retirement_graph <- renderPlotly({
    ...
    plot_ly(brokerage_percentiles(), x = ~age, y = ~`10th_Percentile`, type = "scatter", mode = "lines", name = '10% Percentile', line = list(color='rgb(205, 12, 24)', dash='dot')) %>%
      add_trace(x = ~age, y = ~`25th_Percentile`, name = '25% Percentile', line = list(color='rgb(15, 12, 240)', dash='dot')) %>%
      add_trace(x = ~age, y = ~`Median`, name = 'Median', line = list(color='rgb(0, 255, 0)', dash='solid', size=12)) %>%
      add_trace(x = ~age, y = ~`75th_Percentile`, name = '75% Percentile', line = list(color='rgb(15, 12, 240)', dash='dot')) %>%
      add_trace(x = ~age, y = ~`90th_Percentile`, name = '90% Percentile', line = list(color='rgb(205, 12, 24)'), dash='dot') %>%
